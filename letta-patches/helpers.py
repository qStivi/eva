...
def _build_rule_violation_result(tool_name: str, valid: list[str], solver: ToolRulesSolver) -> ToolExecutionResult:
    ...
    # eva local patch (2026-08-22, see docs/2026-08-22-tool-discovery-spec.md and
    # scripts/register-toolsets.sh): a tool rejected here that isn't a rule violation
    # (no hint_lines) and isn't in `valid` at all is almost always one gated behind
    # search_tools/call_tool's lazy-loading scheme, not attached directly — confirmed
    # live (2026-08-22) that without this, the model tends to read this error as "the
    # tool is broken" and gives up instead of retrying via call_tool. Point it straight
    # at the actual fix instead of leaving that to persona text alone to teach.
    if not hint_lines and "call_tool" in valid and "search_tools" in valid and tool_name not in ("call_tool", "search_tools"):
        hint_txt += (
            f"\n** Hint: '{tool_name}' isn't directly attached — that's expected, not a "
            f"broken tool. If search_tools found it, call it with "
            f"call_tool(name='{tool_name}', args={{...}}) instead of calling it by name "
            f"directly. That works in this same turn; no need to search again first."
        )
