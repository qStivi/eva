...
class OpenAIClient(LLMClientBase):
    ...
    def _apply_mistral_cache_key(self, llm_config: LLMConfig, request_data: dict) -> None:
        """PATCHED 2026-08-13 (local, not upstream): Mistral's prompt caching (90%
        discount on cache hits, matches Anthropic's rate) needs an explicit
        `prompt_cache_key` -- unlike real OpenAI (see _apply_prompt_cache_settings
        above), Mistral's own docs recommend setting one for a reliable hit; without
        it a hit "isn't guaranteed" (confirmed live: an incidental hit happened once
        in testing, but nothing enforces it). Letta's ChatCompletionRequest schema has
        no such field, so inject it via extra_body -- the same mechanism this file
        already uses for OpenRouter reasoning/provider prefs. Keyed on the acting
        user's id: stable across turns for this single-agent setup, and harmless with
        multiple agents since a hit still requires the actual prefix to match.
        """
        if llm_config.provider_name != "mistral" or not self.actor:
            return
        extra = request_data.get("extra_body", {})
        extra["prompt_cache_key"] = f"eva-{self.actor.id}"
    ...
    def build_request_data_responses(
        ...
        # PATCHED 2026-08-13 (local, not upstream): only for actual OpenAI-shaped
        # endpoints. Non-OpenAI BYOK providers routed through this OpenAI-compatible
        # client (e.g. Mistral, model_endpoint_type="openai") have a stricter schema
        # that 422s on an unrecognized `user` field ("extra_forbidden: body.user").
        # Mirrors the existing provider_name-guard pattern used for prompt-caching
        # fields just below/elsewhere in this file. No upstream letta-ai/letta issue
        # found for this as of 2026-08-13.
        if self.actor and llm_config.provider_name in (None, "openai"):
    ...
    def build_request_data(
        ...
        data = ChatCompletionRequest(
            ...
            tool_choice=tool_choice,
            # PATCHED 2026-08-13 (local, not upstream): was `user=str()` -- always set,
            # even for non-OpenAI BYOK providers (Mistral 422s on it). `user` defaults
            # to None on ChatCompletionRequest, so leaving it unset here lets
            # model_dump(exclude_unset=True) omit it unless the provider_name-guarded
            # assignment below explicitly sets it.
        ...
        # always set user id for openai requests
        # PATCHED 2026-08-13 (local, not upstream): see matching comment in
        # build_request_data_responses above. This is the actual path Mistral
        # hits (chat/completions, not the Responses API) -- confirmed live:
        # switching the agent to a mistral/* model and sending a message 422'd
        # with "extra_forbidden: body.user" before this guard was added.
        if self.actor and llm_config.provider_name in (None, "openai"):
            data.user = self.actor.id
        ...
        request_data = data.model_dump(exclude_unset=True)
        self._apply_mistral_cache_key(llm_config, request_data)
